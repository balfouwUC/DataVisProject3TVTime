...
function updateCharacterData(selector, minSeason, maxSeason) {
  ...
  characterElements.forEach(characterElement => {
      ...
      if (characterName) {
        const cd = getCharacterDataForSeasonRange(minSeason, maxSeason, characterName); 
        renderWordCloud(cd); 
          const charData = getPhraseDataForSeasonRange(minSeason, maxSeason, characterName);
...
function getPhraseDataForSeasonRange(firstSeason, lastSeason, characterName) {
  ...
  for (let season = firstSeason; season <= lastSeason; season++) {
    ...
    if (!processedPhrases[seasonKey] || !processedPhrases[seasonKey][characterName]) {
      console.error(`No data available for season ${season} and character ${characterName}.`);
      continue;
    }
...
function renderPhraseCloud(phraseData) {
  ...
  if (!d3.layout || !d3.layout.cloud) {
      console.error("d3.layout.cloud is not available - make sure to load the required library.");
      return;
  }
          ...
          .enter().append("span")
              ...
              .on("mouseover", function(event, d) {
                  const tooltip = d3.select("body").append("div")
                      ...
                      .style("border-radius", "8px")
                      .style("opacity", 0.9)
                      .html(`<strong>Word:</strong> ${d.text}<br><strong>Frequency:</strong> ${d.size}`);
              })
              .on("mouseleave", () => {
                  d3.select(".tooltip").remove();
              });
